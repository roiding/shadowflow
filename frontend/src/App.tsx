...
function App() {
  ...
  const [researchDate, setResearchDate] = useState(localDate())
  ...
  const intradayQuery = useQuery({
    queryKey: ['intraday', boardType, selectedId, researchDate],
    queryFn: async () => (await api.intraday(boardType, selectedId, researchDate)).data ?? [],
  ...
  const qualityQuery = useQuery({ queryKey: ['quality', researchDate], queryFn: async () => (await api.quality(researchDate)).data ?? [], enabled: view === 'quality' })
  const runsQuery = useQuery({ queryKey: ['runs', researchDate], queryFn: async () => (await api.runs(researchDate)).data ?? [], enabled: view === 'quality' })
  ...
  return <div className="app-shell">
    <header className="topbar">
      ...
      <div className="topbar-actions">
        ...
        <div className="live-chip"><span className={`live-dot ${rankQuery.isFetching ? 'is-fetching' : ''}`} />{records.length ? `最新 ${formatTime(records[0].snapshot_at)}` : '等待数据'}</div>
    ...
    <main className="main-content">
      ...
      {view === 'quality' && <QualityView date={researchDate} setDate={setResearchDate} quality={(qualityQuery.data ?? []).filter((item): item is NonNullable<typeof item> & { rank_type: BoardType } => item.rank_type !== 'stock')} runs={runsQuery.data ?? []} loading={qualityQuery.isLoading || runsQuery.isLoading} />}
    </main>
  </div>
}
...
function MonitorView(props: MonitorProps) {
  ...
  return <section className="workspace-grid">
    ...
    <section className={`trend-panel panel-section ${mobilePane === 'ranks' ? 'mobile-hidden' : ''}`}>
	  <div className="section-heading trend-heading"><div><p className="eyebrow">分钟序列</p><h2>{selected?.name ?? '选择一个板块'}</h2><span className="subline">{selected ? `${selected.code} · ${BOARD_LABELS[boardType]} · 采集至 ${formatTime(series.at(-1)?.snapshot_at ?? selected.snapshot_at)}${series.at(-1)?.snapshot_at.slice(11, 16) === '15:00' ? '（日终快照）' : ''}` : '点击左侧榜单查看当日连续序列'}</span></div><a className="export-link" href={selected ? api.exportURL(boardType, selected.code, localDate(), localDate()) : undefined} title="导出研究数据"><Download size={15} />导出</a></div>
