...
“今日监控”的行业/概念表显示 **控盘度排名**，主数字为当前名次，旁边的小标为相对上一交易日收盘的名次变化。

- “暗盘排名”是上游暗盘榜原始排名；“控盘度排名”按控盘度降序计算。
- 估算成交额 = `abs(dark_money) / dark_activity`；活跃度使用接口原始小数。
- 控盘度 = `main_money_inflow / 估算成交额 × 100`，保留两位小数，不带 `%`。主力净流入已经含暗盘，不再叠加一次暗盘资金。
- 今日最新完整截面与前一个交易日的完整收盘截面，分别按控盘度降序排名。行业、概念分开计算，两日均采用上述估算口径；动态筛选和收盘个股使用归档的日终成交额。
- 排名变化 = `昨日控盘排名 - 今日控盘排名`。上升小标显示红色 `↑3`，下降显示绿色 `↓2`，持平显示 `—`；缺少昨日数据时小标显示 `--`，当前名次仍然保留。仅当昨日全榜可用且没有此代码时，显示“新入榜”。悬停可查看比较日期及两日名次。
- 排名使用未四舍五入的控盘度；完全相同值并列（如 `1、1、3`）。搜索、分页及其他列的排序不改变控盘排名，表头按当前控盘度名次排序。
...
最新榜单元数据提供前一交易日，排名计算在前端完成。旧后端未返回比较日期时，当前排名仍可展示，变化小标为 `--`。

今日监控榜单和收盘个股榜的字段按钮支持勾选显示列，默认全部展示，至少保留一列，可恢复默认。两个表格分别保存到浏览器 `localStorage` 的 `shadowflow.monitor.columns.v1` 和 `shadowflow.stocks.columns.v1`，移动端也遵循勾选结果。收盘个股的“控盘度”使用 `main_money_inflow / turnover × 100`，保留两位小数、不带 `%`；资金或成交额不可用时显示 `--`。

## 动态连续筛选

前端“动态筛选”页允许分别为概念和个股添加或删除任意条件，选择“全部满足”或“任一满足”，并配置连续交易日数、是否仅主板、是否排除 ST、个股是否必须属于命中概念。内置初始模板就是原始策略：连续 3 日，概念成交额大于 500 亿元、个股成交额大于 2 亿元，换手率大于 3%，涨幅 1%–6%，控盘度 1.5–6。控盘度按 `(主力明盘 + 主力暗盘) / 成交额 × 100` 即时计算，前端统一显示为两位小数、不带 `%`；API 字段继续使用 `control_coefficient`。
...
通用接口为 `POST /api/v1/focus/scan`。API 使用原始单位：资金/成交额为元，百分比型行情字段为小数，控盘度为已乘 100 的数值；前端会自动换算资金与行情字段的展示单位。
